#sound resources from internet⬇️:
#激光打击音效 许可:CC-BY 作者:nsstudios 来源:耳聆网 https://www.ear0.com/sound/10922
#错误失败/无权限 许可:CC0 作者:野聊 来源:耳聆网 https://www.ear0.com/sound/18729
...
def sound(musicName):
    pygame.init()
    pygame.mixer.init()
    effectSound = pygame.mixer.Sound(musicName)
    effectSound.play()

def music(musicName):
    pygame.init()
    pygame.mixer.init()
    backgroundMusic = pygame.mixer.music.load(musicName)
...
def showLevelUp(level):
    ...
    levelSurf = titleFont.render('LEVEL ' +str(level), True, halfAlphaWhite)
    levelRect = levelSurf.get_rect()
    levelRect.center = (WINDOWWIDTH/2, WINDOWHEIGHT/2)
    draw(levelSurf,levelRect)
